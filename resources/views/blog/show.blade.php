...
{{--            {{trans('messages.welcome', ['name' => 'dayle'])}}--}}
            ...
            <div class="thumbnail container-shadow " style="height: auto; border: 1px dashed #ddd;">
                {{--{{$blog->category->product()->first()->name}}--}}
                <a  href="{{route('giftcard.show',[str_replace(' ', '-', $blog->category->first()->name), str_replace(' ', '-', $blog->category->product()->first()->name)])}}">
                    <h2 class="text-center" style="font-size:22px; color: #3E4F61 !important;">@lang('text.tag_cadeau_tip')</h2>
                    <hr>
                    <h2 class="text-center" style="font-size:18px; color: #3E4F61 !important;">{{$blog->category->product()->first()->name}}</h2>
                    <span class="badge" style="border-radius: 100%;  font-size: 25px; top: 220px; right: 20px; height: 70px; width: 70px; line-height: 60px; position: absolute; background-color:#F59D00;">@lang('text.valuta_sign'){{$blog->category->product()->first()->value}}</span>
                    <img style="padding: 10px;" src="/img/cardlayout/{{$blog->category->product()->first()->category->layout}}" >
                </a>
                <br>
